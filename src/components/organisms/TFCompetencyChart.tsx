...
const SUB_JOBS = ['PI', 'Device', 'FA', 'Photo공정', 'Etch공정', 'Diffusion공정', 'ThinFilm공정', 'C&C공정'];
...
export default function TFCompetencyChart({ template, compact }: Props) {
  ...
  const gaps = SUB_JOBS.map((_, i) => targetScores[i] - actualScores[i]);
  const hasGap = gaps.some((g) => g > 0);
  const height = compact ? 300 : 380;

  const options: Highcharts.Options = {
    chart: { polar: true, type: 'areaspline', height, margin: compact ? [10, 10, 50, 10] : [20, 20, 60, 20], animation: false },
    ...
    yAxis: {
      gridLineInterpolation: 'polygon', min: 0, max: 100, tickInterval: 25,
    ...
    tooltip: {
      ...
      formatter(this: any) {
        ...
        return `<div style="font-size:12px;line-height:1.7"><b>${sj}</b><br/>
    ...
    legend: { enabled: true, layout: 'horizontal', align: 'center', verticalAlign: 'bottom',
      itemStyle: { fontSize: '11px', fontWeight: '600' }, symbolRadius: 6 },
      ...
      {!compact && hasGap && (
        <GapSummary>
          ...
          {gapSummary.map(({ sj, gap }) => (
            <GapItem key={sj}>{sj} <strong>-{gap}pt</strong></GapItem>
          ))}
        </GapSummary>
      )}
    </Wrap>
